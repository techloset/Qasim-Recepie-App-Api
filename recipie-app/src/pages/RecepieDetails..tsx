...
const RecipeDetails: React.FC= () => {
  ...
  const recipeDetails = useSelector((state: RootState) => state.recipe.recipeDetails);
  ...
  return (
    <div>
      <div
        ...
        style={{ backgroundImage: `url(${recipeDetails.strMealThumb})`, height: 400 }}
      >
        <div
      ...
      <div className='top-16 py-12'>
        <div>
          <h1 className='text-3xl font-bold p-5 mx-8 text-start'>Ingredients</h1>
          <ul className='flex flex-wrap p-5 m-5 flex-col items-start justify-start'>
            ...
            {Array.from({ length: 10 }, (_, index) => index + 1).map((index) => {
              ...
              const ingredient: string | null | undefined = recipeDetails?.[ingredientKey];
              const measure: string | null | undefined = recipeDetails?.[measureKey];
      ...
      <div className='top-8 py-4'>
        <div>
          <h1 className='text-3xl font-semibold p-5 mx-8 text-start'>Instructions</h1>
          <ol className='list-decimal list-inside flex flex-wrap p-5 m-5 flex-col items-start justify-start'>
            {recipeDetails.strInstructions.split('\n').map((instruction: string, index: number) => (
              <li className='mx-8 p-1 text-wrap lg:text-xl  sm:text-sm sm:text-start ' key={index}>{instruction.slice(0, 45)}</li>
            ))}
          </ol>
        </div>
      </div>
