...
const SearchResultsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.query) {
      const savedQuery = location.state.query;
      setQuery(savedQuery);
      dispatch(fetchSearchResults(savedQuery));
    }
    ...
    <>
      <div>
        <div className="flex flex-col items-center justify-center">
          <div className="py-4">
            <h1 className="text-2xl font-bold tex p-3">Search Recipes</h1>
            <div className="flex items-center justify-center py-5">
              <div className="border-1 rounded-3xl bg-stone-100 border hover:border-yellow-300 py-3 px-5 flex flex-row justify-between items-center">
                <button onClick={handleSearch}>
                  <img src={Images.searchIcon} alt="" className="me-5" />
                </button>
                <input
                  type="text"
                  value={query}
        ...
        <div className="flex py-2  m-3 ">
          <h1 className="text-start text-3xl font-semibold">Search Results</h1>
        </div>
